...
void setPlayerPosition(int number, float x, float y, float z,
                       float playerroty) {
...
void createTube(int number, float bx, float by, float bz, float ex, float ey,
                float ez, int colour) {
...
void display(void) {
  ...
  if (displayAllCubes == 1) {
    ...
    for (i = 0; i < displayCount; i++) {
      drawCube(displayList[i][0], displayList[i][1], displayList[i][2]);
    }
  }
...
int setUserColour(int id, GLfloat ambRed, GLfloat ambGreen, GLfloat ambBlue,
  ...
  if (id >= NUMBERCOLOURS) {
    printf("ERROR, attempt to setUserColour() with a colour number of %d which "
           "is greater than the maximum user colour number %d.\n",
           id, NUMBERCOLOURS - 1);
    return (1);
  }
...
void unsetUserColour(int id) { uColourUsed[id] = 0; }
