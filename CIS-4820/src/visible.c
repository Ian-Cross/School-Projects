...
float lengthTwoPoints(float x1, float y1, float z1, float x2, float y2,
  ...
  float result;
  result =
      sqrtf(powf((x1 - x2), 2.0) + powf((y1 - y2), 2.0) + powf((z1 - z2), 2.0));
  return (result);
}
...
void cross(float x1, float y1, float z1, float x2, float y2, float z2, float *x,
           float *y, float *z) {
  *x = (y1 * z2) - (z1 * y2);
  *y = (x1 * z2) - (z1 * x2);
  *z = (x1 * y2) - (y1 * x2);
...
double finddet(double a1, double a2, double a3, double b1, double b2, double b3,
  ...
  return ((a1 * b2 * c3) - (a1 * b3 * c2) - (a2 * b1 * c3) + (a3 * b1 * c2) +
          (a2 * b3 * c1) - (a3 * b2 * c1));
}

void intersect(float a1, float b1, float c1, float d1, float a2, float b2,
               float c2, float d2, float a3, float b3, float c3, float d3,
...
void tree(float bx, float by, float bz, float tx, float ty, float tz,
  ...
  if (CubeInFrustum(bx + ((tx - bx) / 2), by + ((ty - by) / 2),
                    bz + ((tz - bz) / 2), length)) {
    if (level == OCTREE_LEVEL) {
      ...
      for (i = bx; i < tx + 1; i++)
        for (j = by; j < ty + 1; j++)
          for (k = bz; k < tz + 1; k++) {
            if ((i < WORLDX) && (j < WORLDY) && (k < WORLDZ) && (i > -1) &&
                (j > -1) && (k > -1))
              if ((world[i][j][k] != 0) &&
