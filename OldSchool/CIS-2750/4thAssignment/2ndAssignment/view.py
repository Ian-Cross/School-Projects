...
def streamChoice(username,filelist):
    notEmpty = os.system("./db -select \"select streamName from streams where binary username='" + username + "'\"")
    if (notEmpty):
        print("All")
    else:
        print("Create Stream")
...
def getLastRead(username,streamName,filelist):
    result = os.system("./db -select \"select lastRead from streams where binary streamName=\'" + streamName + "\'\" > temp.txt")
