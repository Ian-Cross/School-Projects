...
void printFileHeader(){
    printf("<?PHP\n"
                ...
                "\t} else if (isset($_POST['nextPost'])){\n"
                    ...
                    "\t\tif ($currentPost >= totalPost($_GET['stream'])) $currentPost = $_GET['currPost'];\n"
                    "\t\t$textArea = updateViewer($_GET['username'],$_GET['stream'],$currentPost);\n"
                ...
                "\t\t$currPost = getLastPost($_GET['stream']);\n"
                ...
                "\tfunction totalPost($streamName) {\n"
                    "\t\texec(\"./2ndAssignment/viewAPI.py getTotalPosts \".$streamName,$output,$status);\n"
                    "\t\tif ($status) {\n"
                        "\t\t\techo \"Get total Post \".$status;\n"
                    "\t\t}\n"
                    "\t\treturn ( (int)$output[0] );\n"
                "\t}\n\n"
                ...
                "\tfunction updateViewer($username,$streamName,$currentPost) {\n"
                    "\t\texec(\"./2ndAssignment/viewAPI.py getPost \".$username.\" \".$currentPost.\" \".$streamName,$textArea,$status);\n"
                    "\t\tif ($status) {\n"
                        "\t\t\techo \" update Viewer \".$textArea.\": :\".$status;\n"
                    "\t\t}\n"
                    "\t\treturn $textArea;\n"
                "\t}\n\n"
