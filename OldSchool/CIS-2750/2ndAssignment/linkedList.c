...
List* createList(){
  ...
  if (newList == NULL) {
      /* quits when there isn't enough memory */
...
Item* createItem(char* data){
    ...
    newItem->name = malloc(sizeof(char)*(strlen(data) + 1));
...
Prog* createProgram(List* data){
    ...
    if (programInfo == NULL){
        ...
        while (programInfo != NULL){
            ...
            } else if (isType(programInfo->name,NULL) && strcmp(programInfo->name,"*") != 0){
...
Class* createClass(Item* data, int links){
    ...
    int i;
    for (i = 3; i < links; i++){
        if (isType(classInfo->name,NULL) && strcmp(classInfo->name,"*") != 0){
...
Function* createFunction(Item* data, int links){
    ...
    if (funcInfo == NULL){
        ...
        while (isType(funcInfo->name,NULL) || strcmp(funcInfo->name,"*") == 0){
            strcat(newFunc->type," ");
...
int isType(char* data,char* newType){
    ...
    char* defaultTypes[19] = {"PostEntry","*","UserPost","int","char","long","short","double","float","signed","unsigned", "extern", "register", "auto", "static", "const","void","typedef","volatile"};
    int i;
...
Prog* analyze(Prog* myProgram){
    ...
    while (tempClass != NULL){
        ...
        while (tempFunc != NULL){
            ...
            /* Cycling through the guts of the function */
            while (data != NULL){

                /* Finding variables */
                if (isType(data->name,NULL)){
                    while(isType(data->name,NULL) || strcmp(data->name,"*") == 0){
                        data = data->nextItem;
                    }
            ...
            tempFunc = classReference(tempFunc,myProgram->classes,tempClass);
            tempFunc = tempFunc->nextFunc;
    ...
    while (globalFuncs != NULL){
        globalFuncs = classReference(globalFuncs,myProgram->classes,NULL);
...
Function* classReference(Function* theFunc, Class* classes, Class* thisClass){
    ...
    while (data != NULL){
        ...
        } else if (strcmp(data->name,"this") == 0) {
            data->name = realloc(data->name,sizeof(char)*8);
            strcpy(data->name,"newPost");
            data = data->nextItem;
            data->name = realloc(data->name,sizeof(char)*3);
            strcpy(data->name,"->");
            data = data->nextItem;
            Function* tFunc = thisClass->localFuncs;
            while (tFunc != NULL) {
                if (strcmp(tFunc->oldName,data->name) == 0){
                    data->name = realloc(data->name,sizeof(char)*strlen(tFunc->name) + 1);
                    strcpy(data->name,tFunc->name);
                    strcat(data->name,"\0");
                    data = data->nextItem;
                    if (strcmp(data->name,"(") == 0){
                        if (tFunc->classVar == 1){
                            data = data->nextItem;
                            char* pName = malloc(sizeof(char)*(strlen(thisClass->name)+ 4));
                            if (strcmp(data->name,")") == 0) {
                                strcpy(pName,"&");
                            } else {
                                strcpy(pName,", &");
                            }
                            strcat(pName,thisClass->name);
...
Class* renameFuncs(Class* parent){
    ...
    while (localFuncs != NULL){
        ...
        newName[count] = '\0';
        localFuncs->name = realloc(localFuncs->name,sizeof(char)*strlen(newName)+5);
        strcpy(localFuncs->name,newName);
        free(newName);
